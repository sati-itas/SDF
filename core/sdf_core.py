...
class Action(Thing):
    ...
    def execute_dlist(self, debug=False):
        ...
        for select_dict in self.select_dict_list:
            _new_graph = self.Wrapper.copy_rdf_graph()
            if debug:
                print(f'select_dict: {select_dict}')
                print_graph = self.Wrapper.gen_sd_scene_from_rdf_database(_new_graph)
                ...
                for pred, select_parameters in d_dictonary.items():
                    ...
                    for sd_pred, rdf_sub_obj in rdf_rel.items():
                        ...
                        try:
                            ...
                            _new_graph = self.Wrapper.remove_triplets(_new_graph, [triplet])
                            # if debug:
                ...
                for pred, select_parameters in a_dictonary.items():
                    ...
                    for sd_pred, rdf_sub_obj in rdf_rel.items():
                        ...
                        try:
                            ...
                            _new_graph = self.Wrapper.add_triplets(_new_graph, [triplet])
                        except Exception as e:
    ...
    def execute_select_dict_list(self, scene: Scene, debug=False):
        ...
        if self.check_precondition(scene, debug=False):
            ...
            for graph in new_graph_list:
                new_scene = self.Wrapper.gen_sd_scene_from_rdf_database(graph)
    ...
    def execute_select_dict_single(self, scene: Scene, debug=False):
        '''executes the action in a given scene if possible.
        ...
        Returns:
            [new_scene, sd_rel_action_effect] (List[Scene,{Predicate:[SD subject,SD object]}):
            A List includes new scene created by executing action and Dict of the effect by executing action.
            If preconditions for action not fullfilled return Bool:False.
        '''
        self.effect_execute_processing_time = 0.0
        ...
        if self.check_precondition(scene, debug=False):
            ...
            end_effect_execute = timeit.default_timer()
            self.effect_execute_processing_time = end_effect_execute - start_effect_execute
            return [new_scene, sd_rel_action_effect]
        else:
            return [False, False]
