...
class Solver:
    ...
    def __init__(
        ...
        rdf_graph_rules=None,
        predicates: Dict[str, Predicate] = None,
        current_scene_rdf_wrapper: RDFWrapper = None,
        fo_rewrite=False,
    ):
        ...
        if object_template is not None:
            ...
            if len(object_template) == 1:
                ...
                logger.warning(
                    'goal_template is set to object_template[0]. This is deprecated. '
                ...
                raise ValueError(
                    'object_template must be a list containing one or two lists (for object and goal template).'
                )
        else:
        ...
        if fo_rewrite and self.current_scene_rdf_wrapper is None:
            raise Exception(
                '[SDF.SOLVER] First-order logic rewriting is enabled but no current_scene_rdf_wrapper is provided.'
    ...
    def initialize_rdf(
        ...
        if isinstance(current_scene, Scene) and isinstance(goal_scene, Scene):
            logger.info(
                '[SDF.SOLVER.initialize_rdf] Initializing RDF graphs from SDScenes.'
            )
            ...
            logger.info(
                '[SDF.SOLVER.initialize_rdf] RDF graphs are already initialized.'
            )
            goal_rdf_graph = goal_scene
            if self.rdf_graph_rules is not None:
                logger.info(
                    '[SDF.SOLVER.initialize_rdf] Applying RDF graph rules to the current scene graph.'
                )
                current_scene_rdf_graph = self.current_scene_rdf_wrapper.apply_rules(
                    current_scene, rules=self.rdf_graph_rules
                )
            else:
                current_scene_rdf_graph = current_scene
            # Initialize actions with the current scene's RDF wrapper
            for action in action_list:
                action.init_action_with_rdf(
                    self.current_scene_rdf_wrapper, rewrite=self.fo_rewrite
                )
        else:
            raise TypeError(
                'current_scene and goal_scene must both be either Scene or Graph instances.'
            )
    ...
    def dfs_rdf(
        ...
        Returns:
            plan, solution (Tuple[List[Any], bool]): returns a tuple
            with list of actions and a bool which indicates if solution was found
        """
        plan = []
        queue = []
        solution = False
    ...
    def astar_rdf(
        ...
        if RDFUtils.is_subset(goal_scene, current_scene):
            return (plan, True)
        # astar without heuristic is equivalent to uniform cost search
