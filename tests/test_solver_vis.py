...
def test_scenes_vis_execute_statistic(testloop):
    ...
    print(
        f'[mean_bfs_planning_step_processing_time_6]: {np.mean(np.array(data_bfs_6.planning_processing_time))*1000} [msec]'
    )
    print(
        f'[mean_bfs_planning_step_processing_time_10]: {np.mean(np.array(data_bfs_10.planning_processing_time))*1000} [msec]'
    )
    print(
        f'[mean_bfs_planning_step_processing_time_16]: {np.mean(np.array(data_bfs_16.planning_processing_time))*1000} [msec]'
    )
    print(
        f'[mean_bfs_planning_step_processing_time_20]: {np.mean(np.array(data_bfs_20.planning_processing_time))*1000} [msec]'
    )
...
def test_scenes_vis_solver_statistic(testloop):
    ...
    while loop_count < testloop:
        ...
        plan_dfs_6, dfs_planning_step_processing_time_6 = simple_dfs(
            current_scene_6, goal_scene_6, action_list_6, data_logger=DataStorage('simple_dfs')
        )
        plan_dfs_10, dfs_planning_step_processing_time_10 = simple_dfs(
            current_scene_10, goal_scene_10, action_list_10, data_logger=DataStorage('simple_dfs')
        )
        plan_dfs_16, dfs_planning_step_processing_time_16 = simple_dfs(
            current_scene_16, goal_scene_16, action_list_16, data_logger=DataStorage('simple_dfs')
        )
        plan_dfs_20, dfs_planning_step_processing_time_20 = simple_dfs(
            current_scene_20, goal_scene_20, action_list_20, data_logger=DataStorage('simple_dfs')
        )
        ...
        plan_bfs_dp_6, bfs_dp_planning_step_processing_time_6 = BFS_dp(
            current_scene_6, goal_scene_6, action_list_6, data_logger=DataStorage('bfs_dp')
        )
        plan_bfs_dp_10, bfs_dp_planning_step_processing_time_10 = BFS_dp(
            current_scene_10, goal_scene_10, action_list_10, data_logger=DataStorage('bfs_dp')
        )
        plan_bfs_dp_16, bfs_dp_planning_step_processing_time_16 = BFS_dp(
            current_scene_16, goal_scene_16, action_list_16, data_logger=DataStorage('bfs_dp')
        )
        plan_bfs_dp_20, bfs_dp_planning_step_processing_time_20 = BFS_dp(
            current_scene_20, goal_scene_20, action_list_20, data_logger=DataStorage('bfs_dp')
        )
...
def test_scenes(testloop):
    ...
    while loop_count < testloop:
        ...
        plan_bfs_dp, bfs_dp_planning_step_processing_time = bfs_dp(
            current_scene, goal_scene, action_list, data_logger=data_bfs_dp
        )
    ...
    # print(f'[mean_dfs_planning_processing_time :]: {np.mean(np.array(data_simple_dfs.planning_processing_time))*1000} [msec]')
    print(
        f'[mean_bfs_planning_processing_time :]: {np.mean(np.array(data_simple_bfs.planning_processing_time))*1000} [msec]'
    )
    print(
        f'[mean_bfs_dp_planning_processing_time :]: {np.mean(np.array(data_bfs_dp.planning_processing_time))*1000} [msec]'
    )
...
def plot_execute_statistic(datastorage: List[DataStorage]):
    ...
    plt.savefig(
        "Performanz_Aktionsvorlagen-.pdf"
    )  # https://tex.stackexchange.com/questions/508567/best-matplotlitb-to-tikz-conversion-tool
    plt.show()
